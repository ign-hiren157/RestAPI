...
//            .extract().response();
//
//        Assert.assertEquals(response.getStatusCode(), 201, "Expected status code 201 for valid request");
...
//        String requestBody = buildRequestBody("9001015009087", "99999999"); // Invalid deal ID
//
//        Response response = given()
//            .auth().preemptive().basic(USERNAME, PASSWORD)
//            .contentType(ContentType.JSON)
...
//            .extract().response();
//
//        Assert.assertNotEquals(response.getStatusCode(), 201, "Expected failure for invalid deal ID");
//        String error = response.jsonPath().getString("errorMessage");
//        Assert.assertNotNull(error, "Expected error message for invalid deal ID");
